...
async def handle_subject(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("""Пожалуйста, укажите следующие данные: 
— Тип работы (контрольная, зачет, диф. зачет, экзамен, домашняя работа и тд.)
— Если эта работа происходит на паре, то укажите точную дату и время начала пары, чтобы наш сотрудник мог связаться с вами и вовремя решить Ваши задания. В назначенное время в чат с ботом, после отправки Ваших заданий, исполнитель будет присылать решения. 
— Обязательно указать ДЕДЛАЙН (дата и время).
— Максимально конкретно описать ТИП задания.
— Дополнительно: указать своего преподавателя.

💰 Оплата производится по ПОЛНОЙ предоплате.
⏰ Если оплата не внесена в течение 2-х часов после уведомления о принятии задачи, исполнитель не приступает к её решению.""")
