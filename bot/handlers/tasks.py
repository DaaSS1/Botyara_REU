...
import logging
from bot.keyboards import prices
...
async def handle_subject(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📌 При вводе текста задачи необходимо:\n\n"
    "— Обязательно указать ДЕДЛАЙН (дата и время).\n"
    "— Максимально конкретно описать ТИП задания.\n"
    "— Опционально: указать своего преподавателя.\n\n"
    "⚠️ Если у вас экзамен / пересдача / дифзачёт / зачёт / контрольная работа:\n"
    "1️⃣ Создайте первую заявку, где укажите, что у вас одно из этих мероприятий, "
    "и ОБЯЗАТЕЛЬНО добавьте точное время (дату и часы).\n"
    "2️⃣ Если исполнитель принимает заявку, то в указанное время он будет ожидать "
    "от вас вторую заявку с фотографиями заданий.\n"
    "3️⃣ После этого исполнитель в кратчайшие сроки пришлёт готовое решение.\n\n"
    "💰 Оплата производится по ПОЛНОЙ предоплате.\n"
    "⏰ Если оплата не внесена в течение 2-х часов после уведомления о принятии задачи, "
    "исполнитель не приступает к её решению.")
